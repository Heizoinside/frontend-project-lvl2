...
const plainOperations = {
    unchanged: () => null,
    changed: (node, filepath) => `Property ${filepath}${node.name} was changed from ${node.before} to ${getValueContent(node.after)}`,
    added: (node, filepath) => `Property ${filepath}${node.name} was added with value ${getValueContent(node.after)}`,
